define(["utils"], function(utils) {
    return function(data, x, y){
        ...
        title = utils.createParagraph("sectionTitle", "Mixer");
        container.appendChild(title);

        for(var c in channels){

            var levelPropert = channels[c].properties.level;
            var channel = utils.createElement("div", "mixerChannel", 51 * i + 2, 30, "#456"),
                title = utils.createParagraph("propertyTitle", c),
